...
const { reject } = require('../../util/cancelablepromise');
...
class PeerConnectionV2 extends StateMachine {
  ...
  constructor(id, encodingParameters, preferredCodecs, options) {
    ...
    let peerConnection = new window.CitrixWebRTC.CitrixPeerConnection(configuration);
    ...
    peerConnection.connectionstatechange = this._handleConnectionStateChange.bind(this);
    peerConnection.datachannel = this._handleDataChannelEvent.bind(this);
    ...
    peerConnection.signalingstatechange = this._handleSignalingStateChange.bind(this);
    ...
    peerConnection.onaddstream = this._handleStreamEvent.bind(this);
  ...
  _answer(offer) {
    ...
    return Promise.resolve().then(() => {
      ...
      return this._peerConnection.createAnswer((answer) => promiseAnswer = answer);
    }).then(answer => {
      if (isFirefox) {
   ...
   */
  _offer() {
    console.log('banana does offer get called?', this._peerConnection);
    const offerOptions = Object.assign({}, this._offerOptions);
    let sdpConstraints = { mandatory: {
      OfferToReceiveAudio: true,
      OfferToReceiveVideo: true }
    };
    let peerConnection = this._peerConnection;
    ...
    return Promise.all(this._replaceTrackPromises.values())
    // .then(() => {
    //   console.log('banana does this even get called?', this._peerConnection);
    //   return this._peerConnection.createOffer((answer) => {
    //     console.log('banana answer', answer);
    ...
    //   }
    // , 
    // (error) => {console.log('banana is this failing?', error)}, { mandatory: {OfferToReceiveAudio: true, OfferToReceiveVideo: true }})})
  ...
  _setLocalDescription(description) {
    ...
    const setDescription = () => new Promise((resolve) => {
  ...
  _updateDescription(description) {
    switch (description.type) {
      ...
      case 'create-offer':
        ...
        this._descriptionRevision = description.revision;
        console.log('banana hello this is getting here');
        return this._offer();
      case 'offer':
        if (description.revision <= this._lastStableDescriptionRevision
